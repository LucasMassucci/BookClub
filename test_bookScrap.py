...
def test_scrapy_rows(driver=driver):
    assert bookScrap.scrapy_rows(driver) != None

def test_scrapy_rows2(driver=driver):
    driver.get('http://books.toscrape.com/index.html')
    rows = bookScrap.scrapy_rows(driver)
...
def test_extract_data(driver=driver):
    driver.get('http://books.toscrape.com/index.html')
    rows = bookScrap.scrapy_rows(driver)
    data = bookScrap.extract_data(rows, 'All products')
...
def test_to_postgress():
    ...
    bookScrap.to_postgres(data,engine, schema='book_club_basic', table='teste')
...
def test_next_page_extration1(driver=driver):
    driver.get('http://books.toscrape.com/index.html')
    next_page = bookScrap.extract_next_page(driver)
